...
/**
 * @file
 * Theme settings for the gesso_microsites theme.
 */

/**
 * Implements hook_form_system_theme_settings_alter().
 */
function gesso_microsites_form_system_theme_settings_alter(&$form, &$form_state, $form_id = NULL) {
  ...
  $recommended_modules = gesso_microsites_recommended_modules();
  ...
  if (!empty($recommended_modules)) {
    ...
    $form['recommended_modules']['hide_recommended_modules'] = array(
      ...
      '#ajax' => array(
        'callback' => 'gesso_microsites_ajax_settings_save',
      ),
      '#default_value' => $hide,
    );
 ...
 */
function gesso_microsites_ajax_settings_save($form, $form_state) {
  $theme = isset($form_state['build_info']['args'][0]) ? $form_state['build_info']['args'][0] : '';
 ...
 */
function gesso_microsites_recommended_modules() {
  $modules = array();
